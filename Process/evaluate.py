...
def evaluation4class(prediction, y):  # 4 dim
    TP1, FP1, FN1, TN1 = 0, 0, 0, 0  #真正例，假正例，假反例，真反例
    ...
    if (TP1 + FP1)==0:
        ...
        Prec1 = round(float(TP1) / float(TP1 + FP1), 4)  #查准率：预测为真，实际为真的比率
    if (TP1 + FN1 )==0:
        Recll1 =0
    else:
        Recll1 = round(float(TP1) / float(TP1 + FN1 ), 4)  #查全率：实际为真，预测为真的比率
    if (Prec1 + Recll1 )==0:
        F1 =0
    else:
        F1 = round(2 * Prec1 * Recll1 / (Prec1 + Recll1 ), 4) #F1是基于查准率和查全率的调和平均
