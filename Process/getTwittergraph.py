    ...
    for index_i in range(len(index2node)):
        for index_j in range(len(index2node)):
            if index2node[index_i+1].children != None and index2node[index_j+1] in index2node[index_i+1].children:
                ...
                row.append(index_i)
                col.append(index_j)
            # 若a个节点（第一个内容 1:2，2:1），x_index有a个元素为[[1,2],[],...],x_word=[[2,1],[],...]
...
# a个节点，字向量5000，size(x)=[a,5000],ij为第i个节点第j个词频数
...
def main(obj):
    ...
    def loadEid(event,id,y):
        ...
        if len(event) < 2:
            return None
        if len(event)>1:
            x_word, x_index, tree, rootfeat, rootindex = constructMat(event)  #传的是treeDic[eid]
            ...
            # 设所有节点均与root相连
            no = tree[0]!=rootindex
            newedgeindex = rootindex*np.ones((2,len(tree[0][no])))
            newedgeindex[1] = tree[1][no]
            newedgeindex = np.concatenate((tree,newedgeindex),axis=1)
            
            np.savez( os.path.join(cwd, 'data/'+obj+'graph/'+id+'.npz'), x=x_x,root=rootfeat,edgeindex=tree,rootindex=rootindex,y=y,newedgeindex=newedgeindex)
            return None
...
if __name__ == '__main__':
    # obj= sys.argv[1] #获取运行python文件的时候第1个命令行参数
    # main('Twitter15')
    main('Twitter16')